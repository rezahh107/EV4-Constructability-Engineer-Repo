...
SCHEMA_BY_KIND = {'constructability_review': 'schemas/constructability_review.schema.json', 'implementation_strategy_map': 'schemas/implementation_strategy_map.schema.json', 'builder_executable_package': 'schemas/builder_executable_package.schema.json'}
REFERENCE_PARADIGM_SCHEMA = 'schemas/reference-paradigm-lock.schema.json'
VERIFIED_CE_PAYLOAD_SCHEMA_ID = 'ev4-ce-stage-payload@1.1.0'
VERIFIED_CE_PAYLOAD_SCHEMA = 'schemas/ce_stage_payload.v1_1.schema.json'
...
def _collect_schema_errors(schema: dict[str, Any], document: Any, prefix: str) -> list[str]:
    validator = Draft202012Validator(schema)
    errors: list[str] = []
    for error in sorted(validator.iter_errors(document), key=lambda item: tuple((str(part) for part in item.path))):
...
def schema_validate(document: dict[str, Any], repo_root: Path | None=None) -> list[str]:
    ...
    if document.get('schema_id') == VERIFIED_CE_PAYLOAD_SCHEMA_ID:
        schema = load_json(root / VERIFIED_CE_PAYLOAD_SCHEMA)
        review_schema = load_json(root / 'schemas/constructability_review.v1_1.schema.json')
        ...
        for error in sorted(validator.iter_errors(document), key=lambda item: tuple((str(part) for part in item.path))):
            path = '.'.join((str(part) for part in error.path)) or 'ce_stage_payload'
            errors.append(f'{path}: {error.message}')
        return errors
    errors: list[str] = []
...
def _mode_preflight_violations(document: dict[str, Any], mode: ValidationMode) -> list[ConstructabilityViolation]:
    ...
    if mode == 'report' and review.get('constructability_status') == 'executable_ready':
        return [ConstructabilityViolation(rule_id='R21_REPORT_MODE_MUST_BE_NON_EXECUTABLE', status='blocked', message='report mode validates non-executable reviews only; status must not be executable_ready.', location='constructability_review.constructability_status')]
    return []

def _visual_parity_requested(document: dict[str, Any]) -> bool:
    package = document.get('builder_executable_package')
    return bool(document.get('visual_parity_build') is True or document.get('builder_ready') is True or (isinstance(package, dict) and package.get('visual_parity_build') is True))
...
def _package_carrier_violations(document: dict[str, Any]) -> list[ConstructabilityViolation]:
    ...
    if not isinstance(package.get('reference_paradigm_lock'), dict) or package['reference_paradigm_lock'].get('paradigm_locked') is not True:
        violations.append(ConstructabilityViolation(rule_id='R35_REFERENCE_PARADIGM_LOCK_MUST_BE_CARRIED_IN_PACKAGE', status='blocked', message='visual-parity Builder package must carry locked reference_paradigm_lock inside builder_executable_package.', location='builder_executable_package.reference_paradigm_lock'))
    if not isinstance(package.get('paradigm_to_structure_map'), dict):
        violations.append(ConstructabilityViolation(rule_id='R36_REFERENCE_PARADIGM_STRUCTURE_MAP_MUST_BE_CARRIED_IN_PACKAGE', status='blocked', message='visual-parity Builder package must carry paradigm_to_structure_map inside builder_executable_package.', location='builder_executable_package.paradigm_to_structure_map'))
    return violations
...
def _reference_paradigm_preflight_violations(document: dict[str, Any]) -> list[ConstructabilityViolation]:
    if not _visual_parity_requested(document):
        return []
    violations = _package_carrier_violations(document)
    if violations:
        return violations
    lock, structure_map = _reference_carriers(document)
    if not isinstance(lock, dict) or lock.get('paradigm_locked') is not True:
        violations.append(ConstructabilityViolation(rule_id='R29_REFERENCE_PARADIGM_LOCK_REQUIRED', status='blocked', message='visual-parity Builder-ready output requires locked reference_paradigm_lock.', location='reference_paradigm_lock'))
        return violations
    if lock.get('layout_paradigm') == 'unknown':
        violations.append(ConstructabilityViolation(rule_id='R30_REFERENCE_PARADIGM_UNKNOWN_BLOCKS_BUILDER_READY', status='blocked', message='layout_paradigm unknown blocks Builder-ready visual-parity output.', location='reference_paradigm_lock.layout_paradigm'))
    if not isinstance(structure_map, dict):
        violations.append(ConstructabilityViolation(rule_id='R31_REFERENCE_PARADIGM_STRUCTURE_MAP_REQUIRED', status='blocked', message='visual-parity Builder-ready output requires paradigm_to_structure_map.', location='paradigm_to_structure_map'))
    elif not isinstance(structure_map.get('first_batch_requirements'), list) or not structure_map.get('first_batch_requirements'):
        violations.append(ConstructabilityViolation(rule_id='R32_REFERENCE_PARADIGM_FIRST_BATCH_REQUIREMENTS_REQUIRED', status='blocked', message='visual-parity Builder-ready output requires first_batch_requirements.', location='paradigm_to_structure_map.first_batch_requirements'))
    return violations
...
def validate_document(document: dict[str, Any], *, repo_root: Path | None=None, mode: ValidationMode='full') -> dict[str, Any]:
    ...
    if document.get('schema_id') == VERIFIED_CE_PAYLOAD_SCHEMA_ID:
        return {'passed': not schema_errors, 'mode': mode, 'schema_errors': schema_errors, 'violations': [], 'rules_violated': [], 'expected_pass': None}
    ...
    result: dict[str, Any] = {'passed': passed, 'mode': mode, 'schema_errors': schema_errors, 'violations': [violation.__dict__ for violation in rule_violations], 'rules_violated': [violation.rule_id for violation in rule_violations], 'expected_pass': expected_pass}
