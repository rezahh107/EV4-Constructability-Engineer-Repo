...
def _actions(draft: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        dict(item)
        for item in draft.get("builder_action_proposals") or []
...
def _strategies(draft: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        dict(item)
        for item in proposal.get("strategies") or []
        if isinstance(item, Mapping)
    ]
...
def _effect_records(obligations: Mapping[str, Any]) -> list[dict[str, Any]]:
    effects = obligations.get("action_effects")
    if not isinstance(effects, Mapping):
        return []
    return [dict(item) for item in effects.get("records") or [] if isinstance(item, Mapping)]


def _effect_records_by_target(obligations: Mapping[str, Any]) -> dict[str, list[dict[str, Any]]]:
    result: dict[str, list[dict[str, Any]]] = {}
    for record in _effect_records(obligations):
...
def derive_architecture_identity_preservation(
    ...
    classes = sorted(
        str(value)
        for value in (intent.get("class_intent") or {}).get("approved_class_names") or []
    )
    source_classes = sorted(
        str(value)
        for value in (bundle_intent.get("class_intent") or {}).get("approved_class_names") or []
    )
    ...
    review_coverage_preserved = (
        sorted(set(reviewed_ids)) == sorted(required_nodes)
        and not obligations["duplicate_nodes"]
    )
    if not review_coverage_preserved:
        diagnostics.append({"code": "CE_IDENTITY_BUILD_TREE_REVIEW_COVERAGE_INCOMPLETE"})

    effect_records = _effect_records(obligations)
    class_set_changes = sorted(
        str(item.get("action_id"))
        for item in effect_records
        if item.get("changes_approved_class_set") is True
    )
    structure_changes = sorted(
        str(item.get("action_id"))
        for item in effect_records
        if item.get("changes_structure") is True
    )
    ...
    forbidden_conflicts = sorted(
        {
            str(value)
            for item in effect_records
            for value in item.get("forbidden_work_conflicts") or []
        }
    )

    # Draft flags remain additional disclosure. They cannot suppress an action-derived effect,
    # but a disclosed change with no canonical permission continues to fail closed.
    reviewed = _reviewed_by_id(review_draft)
    disclosed_unpermitted = sorted(
        node_id
        for node_id, node in reviewed.items()
        if (
            node.get("requires_class_change") is True
            or node.get("requires_structure_change") is True
        )
        and node.get("architect_decomposition_permission") is not True
        and not any(
            item.get("target_node") == node_id
            and (
                (item.get("class_effect") or {}).get("permission_granted") is True
                or (item.get("structure_effect") or {}).get("permission_granted") is True
            )
            for item in effect_records
        )
    )

    if class_set_changes:
        diagnostics.append(
            {
                "code": "CE_IDENTITY_BUILDER_ACTION_CHANGES_APPROVED_CLASSES",
                "actions": class_set_changes,
            }
        )
    if structure_changes:
        diagnostics.append(
            {
                "code": "CE_IDENTITY_BUILDER_ACTION_CHANGES_BUILD_TREE",
                "actions": structure_changes,
            }
        )
    if blocked_effects or disclosed_unpermitted:
        diagnostics.append(
            {
                "code": "CE_IDENTITY_UNAUTHORIZED_REDESIGN",
                "actions": blocked_effects,
                "nodes": disclosed_unpermitted,
            }
        )
    if forbidden_conflicts:
        diagnostics.append(
            {
                "code": "CE_IDENTITY_FORBIDDEN_WORK_CONFLICT",
                "forbidden_work": forbidden_conflicts,
            }
        )
    ...
    unknowns_preserved = echoed_unknowns is None or sorted(
        str(value) for value in echoed_unknowns
    ) == unknown_ids
    ...
    forbidden_preserved = echoed_forbidden is None or sorted(
        str(value) for value in echoed_forbidden
    ) == forbidden_work
    ...
    if echoed_nodes is not None and sorted(str(value) for value in echoed_nodes) != sorted(
        required_nodes
    ):
        diagnostics.append({"code": "CE_IDENTITY_DRAFT_BUILD_TREE_CHANGED"})

    effects_by_target = _effect_records_by_target(obligations)
    traces: list[dict[str, Any]] = []
    for item in obligations["required_architect_nodes"]:
        node_id = item["node_id"]
        node_effects = effects_by_target.get(node_id, [])
        identity_changed = any(record.get("changes_structure") is True for record in node_effects)
        traces.append(
            {
                "architect_node_ref": node_id,
                "architect_evidence_refs": list(item.get("evidence_refs") or []),
                "ce_review_unit_id": f"ce-unit-{node_id}",
                "identity_unchanged": (
                    node_id in reviewed
    ...
    # A permitted extension may intentionally set identity_unchanged=false for the affected
    # node. Trace completeness means every canonical node is traced and has source evidence.
    review_trace_complete = (
        len(traces) == len(required_nodes)
        and not missing_trace_evidence
        and all(item["architect_node_ref"] in reviewed for item in traces)
    )
    if not review_trace_complete:
        diagnostics.append({"code": "CE_IDENTITY_REVIEW_TRACE_INCOMPLETE"})

    selected_preserved = candidate == source_candidate and echoed_candidate in {None, candidate}
    classes_preserved = (
        classes == source_classes
        and not class_set_changes
        and (echoed_classes is None or sorted(str(value) for value in echoed_classes) == classes)
    )
    build_tree_preserved = (
        review_coverage_preserved
        and not structure_changes
        and (
            echoed_nodes is None
            or sorted(str(value) for value in echoed_nodes) == sorted(required_nodes)
        )
    )
    result = {
        "result_kind": "architecture_identity_preservation_result",
        "selected_candidate_preserved": selected_preserved,
        "approved_classes_preserved": classes_preserved,
        "build_tree_nodes_preserved": build_tree_preserved,
        "architect_unknowns_preserved": unknowns_preserved,
        "forbidden_work_preserved": forbidden_preserved and not forbidden_conflicts,
        "unauthorized_redesign_absent": not blocked_effects and not disclosed_unpermitted and not forbidden_conflicts,
        ...
        "action_effects": copy.deepcopy(effect_records),
        "class_change_actions": class_set_changes,
        "structure_change_actions": structure_changes,
...
def derive_review_units_and_interrogation_results(
    ...
    for node_id in required:
        ...
        if (
            not item
            or not isinstance(item.get("proposed_action"), str)
            or not isinstance(item.get("engineering_rationale"), str)
        ):
    ...
    if obligations["missing_nodes"]:
        diagnostics.append(
            {
                "code": "CE_REVIEW_UNIT_REQUIRED_NODES_MISSING",
                "nodes": obligations["missing_nodes"],
            }
        )
    if obligations["orphan_nodes"]:
        diagnostics.append(
            {"code": "CE_REVIEW_UNIT_ORPHAN_NODES", "nodes": obligations["orphan_nodes"]}
        )
    ...
    coverage = {
        node_id: list(obligations["required_claims_by_node"].get(node_id, []))
        for node_id in required
    }
    result = {
...
def derive_dependency_classification(
    *,
    ...
    for node_id in sorted(obligations["required_claims_by_node"]):
        for claim_id in obligations["required_claims_by_node"][node_id]:
            evaluation = evaluate_claim(
                ...
                execution_transaction_id_override=execution_transaction,
            )
            candidate = architect_intake.get("selected_architecture", {}).get(
    ...
    expected = sum(
        len(values) for values in obligations["required_claims_by_node"].values()
    )
    ...
    if not rows and not obligations["explicit_no_claim_nodes"]:
        diagnostics.append({"code": "CE_DEPENDENCY_EMPTY_MATRIX_UNPROVEN"})
    blocking = sorted(
        f"{item['subject_ref']}:{item['claim_id']}:{item['status']}"
        for item in rows
        if item["blocking"]
    )
    unresolved = [
    ...
    downstream = [
        item["downstream_obligation"]
        for item in rows
        if item.get("downstream_obligation") is not None
    ]
    evidence = [
        record for item in rows for record in item.get("evidence_records") or []
    ]
    result = {
        ...
        "runtime_execution": execution_batch.as_dict() if execution_batch else None,
        "diagnostics": diagnostics,
    }
    result["status"] = "blocked" if blocking or diagnostics else "complete"
    return _canonical(result)
...
def derive_implementation_strategy_coverage(
    ...
    if missing_strategy_nodes:
        diagnostics.append(
            {"code": "CE_STRATEGY_REQUIRED_NODE_MISSING", "nodes": missing_strategy_nodes}
        )
    if duplicate_strategy_nodes:
        diagnostics.append(
            {"code": "CE_STRATEGY_DUPLICATE_NODE", "nodes": duplicate_strategy_nodes}
        )
    ...
    invalid_targets = sorted(
        str(item.get("action_id"))
        for item in actions
        if item.get("target_node") not in required_nodes
    )
    if invalid_targets:
        diagnostics.append(
            {"code": "CE_STRATEGY_BUILDER_ACTION_TARGET_INVALID", "actions": invalid_targets}
        )
    uncovered_actions = sorted(
        str(item.get("action_id"))
        for item in actions
        if item.get("target_node") not in set(strategy_nodes)
    )
    if uncovered_actions:
        diagnostics.append(
            {"code": "CE_STRATEGY_BUILDER_ACTION_UNCOVERED", "actions": uncovered_actions}
        )

    hidden_paths: list[str] = []
    for index, action in enumerate(actions):
        hidden_paths.extend(
            _contains_unresolved_choice(
                action.get("parameters"), f"$.actions[{index}].parameters"
            )
        )
    ...
    if hidden_paths:
        diagnostics.append(
            {"code": "CE_STRATEGY_HIDDEN_BUILDER_DECISION", "paths": sorted(hidden_paths)}
        )

    effects = _effect_records(obligations)
    blocked_effect_actions = sorted(
        str(item.get("action_id")) for item in effects if item.get("blocked") is True
    )
    if blocked_effect_actions:
        diagnostics.append(
            {
                "code": "CE_STRATEGY_ACTION_EFFECT_BLOCKED",
                "actions": blocked_effect_actions,
            }
        )
    ...
    # Preserve the prior fail-closed disclosure behavior for Draft-only flags.
    reviewed = _reviewed_by_id(review_draft)
    disclosed_nodes = sorted(
        node_id
        for node_id, item in reviewed.items()
        if (
            item.get("requires_class_change") is True
            or item.get("requires_structure_change") is True
        )
    ...
    amendment_nodes = sorted(
        ...
        | {
            ...
            for item in effects
            if str(item.get("action_id")) in permission_required
        }
    )
    if amendment_nodes:
        diagnostics.append(
            {"code": "CE_STRATEGY_ARCHITECT_AMENDMENT_REQUIRED", "nodes": amendment_nodes}
        )
    ...
    classes_preserved = classes_value is None or sorted(
        str(value) for value in classes_value
    ) == list(identity_result.get("approved_class_names") or [])
    classes_preserved = classes_preserved and bool(
        identity_result.get("approved_classes_preserved")
    )
    build_tree_preserved = bool(identity_result.get("build_tree_nodes_preserved"))
    ...
    if not build_tree_preserved:
        diagnostics.append({"code": "CE_STRATEGY_BUILD_TREE_CHANGED"})

    dependency_rows = dependency_result.get("rows") or []
    explicit_no_claim_complete = set(
        obligations.get("explicit_no_claim_nodes") or []
    ) == required_nodes
    non_blocking_covered = (
        bool(dependency_rows)
        and all(
            item.get("status") in {"satisfied", "not_applicable"}
    ...
    first_safe_batch_complete = (
        bool(actions)
        and not invalid_targets
        and not uncovered_actions
        and not hidden_paths
        and not blocked_effect_actions
        and candidate_preserved
        and classes_preserved
        and build_tree_preserved
    )
    result = {
        "result_kind": "implementation_strategy_coverage_result",
        "required_review_units_covered": not missing_strategy_nodes
        and not duplicate_strategy_nodes,
        ...
        "builder_actions_valid": not invalid_targets
        and not uncovered_actions
        and not blocked_effect_actions,
        ...
        "approved_classes_preserved": classes_preserved,
        "build_tree_preserved": build_tree_preserved,
        "strategies": copy.deepcopy(strategies),
        "builder_actions": copy.deepcopy(actions),
        "action_effects": copy.deepcopy(effects),
        "diagnostics": diagnostics,
    }
    result["status"] = (
        "blocked"
        if dependency_result.get("blocking_dependencies") or diagnostics
        else "complete"
    )
    return _canonical(result)
...
def evaluate_all(
    ...
    *,
    ...
    obligations = derive_review_obligations(
