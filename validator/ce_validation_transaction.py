...
def _transaction_authorization_diagnostics(
    ...
    if payload.get("payload_status") != "complete":
        diagnostics.append(
            ExportDiagnostic(
                "CE_TRX_PAYLOAD_STATUS_NOT_COMPLETE",
                "handoff_recomputation",
                "Builder handoff requires payload_status=complete.",
                "$.final_stage_bundle.payload.data.payload_status",
            )
        )
    if payload.get("unresolved_evidence"):
        diagnostics.append(
            ExportDiagnostic(
                "CE_TRX_UNRESOLVED_EVIDENCE",
                "handoff_recomputation",
                "Builder handoff cannot coexist with unresolved CE evidence.",
                "$.final_stage_bundle.payload.data.unresolved_evidence",
            )
        )
    if review.get("constructability_status") not in {
        "executable_ready",
        "executable_with_logged_assumption",
    }:
        diagnostics.append(
            ExportDiagnostic(
                "CE_TRX_CONSTRUCTABILITY_NOT_EXECUTABLE",
                "handoff_recomputation",
                "Builder handoff requires an executable constructability result.",
    ...
    if payload.get("builder_package_emitted") is not True or package is None:
        diagnostics.append(
            ExportDiagnostic(
                "CE_TRX_BUILDER_PACKAGE_NOT_EMITTED",
                "handoff_recomputation",
                "Builder handoff requires an emitted Builder Executable Package.",
                "$.final_stage_bundle.payload.data.builder_package_emitted",
            )
        )
    elif (
        ...
        diagnostics.append(
            ExportDiagnostic(
                "CE_TRX_BUILDER_PACKAGE_NOT_ELIGIBLE",
                "handoff_recomputation",
                "Builder package status, decisions, blockers, or schema are not eligible.",
    ...
    for index, stage in enumerate(stages):
        if not isinstance(stage, dict):
            diagnostics.append(
                ExportDiagnostic(
                    "CE_TRX_MANDATORY_STAGE_INVALID",
                    "handoff_recomputation",
                    "Every transaction stage must be a structured record.",
        ...
        if stage.get("mandatory") is True and stage.get("status") != "complete":
            diagnostics.append(
                ExportDiagnostic(
                    "CE_TRX_MANDATORY_STAGE_INCOMPLETE",
                    "handoff_recomputation",
                    "All mandatory CE transaction stages must be complete before handoff.",
    ...
    if (
        ...
        diagnostics.append(
            ExportDiagnostic(
                ...
                "handoff_recomputation",
                "Allowed handoff cannot contain failure, blocking, or unresolved surfaces.",
                "$.handoff",
            )
        )
    semantic = (
    ...
    if not semantic.get("passed"):
        diagnostics.append(
            ExportDiagnostic(
                "CE_TRX_CE_SEMANTIC_VALIDATION_FAILED",
                "handoff_recomputation",
                "Official CE semantic validation does not authorize this handoff.",
...
def secure_build_export(
    *,
    ...
    payload_snapshot = capture_json_snapshot(
        payload_path,
        label="CE Stage Payload",
        read_error_code="CE_EXPORT_PAYLOAD_READ_FAILED",
        changed_error_code="CE_EXPORT_PAYLOAD_CHANGED_DURING_EXPORT",
    )
