...
from .project_gate_export import CE_REPOSITORY, PIPELINE_ID, validate_producer_gate_export
...
from .project_gate_exporter_core import BUILDER_PACKAGE_SCHEMA_ID, HANDOFF_TARGET, PRODUCER_EXPORT_SCHEMA_ID, ExportDiagnostic, ExporterError, GitProvenance, _json_hash, _reject_non_json_constant, validate_builder_package, validate_identity_preservation, validate_payload_and_ce_semantics, verify_source_intake_binding
...
def capture_json_snapshot(path: Path, *, label: str, read_error_code: str, changed_error_code: str) -> JsonSnapshot:
    try:
        raw = path.read_bytes()
    except OSError as exc:
        raise ExporterError(ExportDiagnostic(read_error_code, 'source_binding', f'Failed to read {label}: {exc}', str(path), 'repository_owner')) from exc
    try:
        value = json.loads(raw.decode('utf-8'), parse_constant=_reject_non_json_constant, object_pairs_hook=_duplicate_keys_forbidden)
    except (UnicodeDecodeError, json.JSONDecodeError, ValueError) as exc:
        raise ExporterError(ExportDiagnostic('CE_EXPORT_INPUT_INVALID_JSON', 'input_snapshot_parse', f'Invalid {label}: {exc}', str(path))) from exc
    if not isinstance(value, dict):
        raise ExporterError(ExportDiagnostic('CE_EXPORT_INPUT_INVALID_JSON', 'input_snapshot_parse', f'Expected JSON object in {path}', str(path)))
    return JsonSnapshot(path=path, value=value, raw_bytes=raw, read_error_code=read_error_code, changed_error_code=changed_error_code, label=label)

def assert_snapshot_unchanged(snapshot: JsonSnapshot) -> None:
    try:
        observed = snapshot.path.read_bytes()
    except OSError as exc:
        raise ExporterError(ExportDiagnostic(snapshot.read_error_code, 'source_binding', f'Failed to re-read {snapshot.label}: {exc}', str(snapshot.path), 'repository_owner')) from exc
    if observed != snapshot.raw_bytes:
        raise ExporterError(ExportDiagnostic(snapshot.changed_error_code, 'source_binding', f'{snapshot.label} changed after its exact bytes were captured.', str(snapshot.path), 'repository_owner'))

def _owned_output(path: Path) -> bool:
    try:
        value = json.loads(path.read_text(encoding='utf-8'), parse_constant=_reject_non_json_constant, object_pairs_hook=_duplicate_keys_forbidden)
    ...
    try:
        ...
        for protected in protected_inputs:
            if resolved == protected.resolve(strict=False):
                raise ExporterError(ExportDiagnostic('CE_EXPORT_OUTPUT_ALIASES_INPUT', 'output_safety', 'Output path must not alias any transaction input.', str(resolved), 'repository_owner'))
        if resolved.exists():
            if not overwrite:
            ...
            if not _owned_output(resolved):
                raise ExporterError(ExportDiagnostic('CE_EXPORT_OUTPUT_NOT_OWNED', 'output_safety', 'Refusing to overwrite an existing target not owned by the CE exporter.', str(resolved), 'repository_owner'))
        return resolved
...
def _synchronize_intake_stage_status(stage_manifest: list[dict[str, Any]], intake_report: dict[str, Any], intake: dict[str, Any]) -> None:
...
def _transaction_authorization_diagnostics(repo_root: Path, export: dict[str, Any]) -> list[ExportDiagnostic]:
    ...
    if not isinstance(declared_synthetic, bool) or declared_synthetic != derived_synthetic:
        diagnostics.append(ExportDiagnostic('CE_TRX_SYNTHETIC_STATE_MISMATCH', 'handoff_recomputation', 'Declared synthetic state does not match validator-derived carried evidence.', '$.final_stage_bundle.synthetic'))
    ...
    elif package.get('schema') != BUILDER_PACKAGE_SCHEMA_ID or package.get('builder_package_status') != 'executable_ready' or package.get('builder_decisions_required') != 0 or (package.get('blocking_dependencies') != []):
    ...
    if handoff.get('failure_reasons') or handoff.get('blocking_diagnostics') or handoff.get('unresolved_evidence'):
        diagnostics.append(ExportDiagnostic('CE_TRX_HANDOFF_SURFACES_CONTRADICT', 'handoff_recomputation', 'Allowed handoff cannot contain failure, blocking, or unresolved surfaces.', '$.handoff'))
...
def validate_transaction_artifact(repo_root: Path, export: dict[str, Any]) -> list[ExportDiagnostic]:
    return _transaction_authorization_diagnostics(repo_root, export)

def secure_build_export(*, repo_root: Path, payload_path: Path, source_intake_path: Path, source_bundle_path: Path, output_path: Path, provenance: GitProvenance) -> tuple[dict[str, Any], dict[str, Any], tuple[ExportDiagnostic, ...]]:
    """Validate a historical Payload, then force a deterministic preview-only handoff boundary."""
    ...
    with orchestration_module._private_validation_snapshots(intake_snapshot.raw_bytes, bundle_snapshot.raw_bytes) as (validator_intake_path, validator_bundle_path):
        intake_report = orchestration_module.run_official_intake_validation(repo_root, validator_intake_path, validator_bundle_path)
    source_hash = verify_source_intake_binding(payload, intake, source_intake_path, intake_snapshot.raw_bytes)
    ...
    summary = {'export_id': export['export_id'], 'source_intake_hash': source_hash['value'], 'source_intake_hash_scope': source_hash['scope'], 'source_bundle_hash': _json_hash(source_bundle), 'ce_payload_hash': _json_hash(payload), 'payload_snapshot_sha256': hashlib.sha256(payload_snapshot.raw_bytes).hexdigest(), 'source_intake_snapshot_sha256': hashlib.sha256(intake_snapshot.raw_bytes).hexdigest(), 'source_bundle_snapshot_sha256': hashlib.sha256(bundle_snapshot.raw_bytes).hexdigest(), 'builder_executable_package_hash': builder_package_hash, 'bundle_hash': bundle_hash, 'export_identity_hash': identity_hash, 'export_hash': _json_hash(export), 'producer_commit': provenance.commit_sha, 'producer_ref': provenance.ref, 'repository_dirty': provenance.dirty, 'dirty_paths': list(provenance.dirty_paths), 'handoff_target': HANDOFF_TARGET, 'intake_validation_status': intake_report['status'], 'artifact_integrity_status': 'valid', 'semantic_validation_status': 'valid', 'authorization_valid': handoff_allowed}
    return apply_legacy_preview_boundary(export, summary, tuple(handoff_diagnostics))
