...
def _structure_nodes(intake: Mapping[str, Any], source_bundle: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in intake_nodes:
        if not isinstance(item, Mapping):
            raise ObligationDerivationError("Architect intake contains an invalid structure node")
        node_id = item.get("source_node_id")
        if not isinstance(node_id, str) or not node_id:
            raise ObligationDerivationError("Architect intake structure node has no source_node_id")
        if node_id in normalized_intake:
            raise ObligationDerivationError(f"Duplicate Architect intake node: {node_id}")
        normalized_intake[node_id] = dict(item)

    normalized_bundle: dict[str, dict[str, Any]] = {}
    for item in bundle_nodes:
        if not isinstance(item, Mapping):
            raise ObligationDerivationError("Architect source bundle contains an invalid structure node")
        node_id = item.get("node_id")
        if not isinstance(node_id, str) or not node_id:
            raise ObligationDerivationError("Architect source bundle structure node has no node_id")
        if node_id in normalized_bundle:
            raise ObligationDerivationError(f"Duplicate Architect source node: {node_id}")
        normalized_bundle[node_id] = dict(item)

    if set(normalized_intake) != set(normalized_bundle):
        raise ObligationDerivationError("Architect intake and source bundle structure identities differ")
    ...
    for node_id in sorted(normalized_intake):
        ...
        if (
            ...
            or list(intake_node.get("children") or []) != list(source_node.get("children") or [])
            or intake_node.get("node_kind") != source_node.get("node_kind")
        ):
            raise ObligationDerivationError(f"Architect node identity differs for {node_id}")
        result.append(
            {
                "node_id": node_id,
                "node_kind": str(source_node.get("node_kind") or "implementation_node"),
                "parent_node_id": source_node.get("parent_node_id"),
                "children": list(source_node.get("children") or []),
                "evidence_refs": sorted(str(value) for value in source_node.get("evidence_refs") or []),
            }
        )
    return result
...
def _requested_claims(node: Mapping[str, Any]) -> set[str]:
    ...
    if unknown:
        raise ObligationDerivationError(f"Unknown requested claims: {', '.join(unknown)}")
    return result
...
def _action_map(draft: Mapping[str, Any]) -> dict[str, list[dict[str, Any]]]:
    ...
    for item in actions:
        ...
        if not all(isinstance(value, str) and value for value in (action_id, action_type, target)):
...
def derive_review_obligations(
    ...
    """Derive the complete CE review matrix from canonical facts and additive Draft requests."""
    ...
    reviewed_ids = [str(item.get("node_id")) for item in reviewed if isinstance(item.get("node_id"), str)]
    counts = Counter(reviewed_ids)
    duplicate_nodes = sorted(node_id for node_id, count in counts.items() if count > 1)
    ...
    for node_id in missing_nodes:
        diagnostics.append({"code": "CE_OBLIGATION_REQUIRED_NODE_MISSING", "node_id": node_id})
    for node_id in orphan_nodes:
        diagnostics.append({"code": "CE_OBLIGATION_ORPHAN_DRAFT_NODE", "node_id": node_id})
    for node_id in duplicate_nodes:
        diagnostics.append({"code": "CE_OBLIGATION_DUPLICATE_REVIEW_UNIT", "node_id": node_id})
    for node_id in unknown_action_targets:
        diagnostics.append({"code": "CE_OBLIGATION_UNKNOWN_ACTION_TARGET", "node_id": node_id})

    reviewed_by_id = {str(item.get("node_id")): item for item in reviewed if isinstance(item.get("node_id"), str)}
    ...
    for node in required_nodes:
        ...
        if draft_node.get("requires_class_change") is True or draft_node.get("requires_structure_change") is True:
            mandatory.add("geometry")
        requested = _requested_claims(draft_node)
        all_claims = mandatory | requested

        # An empty set is valid only when every action is explicitly mapped to no claims and
        # there are no semantic hints or change flags. It is recorded, not inferred via all([]).
        explicit_no_claim = bool(actions) and all(
            derive_action_claims(str(action["action_type"])) == () for action in actions
        )
        if not actions:
            diagnostics.append({"code": "CE_OBLIGATION_NODE_HAS_NO_BUILDER_ACTION", "node_id": node_id})
        elif not all_claims and explicit_no_claim:
            explicit_no_claim_nodes.append(node_id)
        elif not all_claims:
            diagnostics.append({"code": "CE_OBLIGATION_EMPTY_CLAIM_SET_UNPROVEN", "node_id": node_id})
        ...
        applicable_rules_by_node[node_id] = sorted(
            str(claim_policy_registry[claim]["applicable_rule"]) for claim in all_claims
        )
    ...
    complete = not any(item["code"] in blocking_codes for item in diagnostics)
    return {
        ...
        "required_builder_decision_checks": sorted(
            str(action["action_id"]) for actions in actions_by_node.values() for action in actions
        ),
        "required_architect_permissions": sorted(
            node_id
            for node_id, item in reviewed_by_id.items()
