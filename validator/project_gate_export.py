...
def _schema_errors(
    ...
    for error in sorted(validator.iter_errors(document), key=_schema_error_sort_key):
        ...
        diagnostics.append(
            Diagnostic("CE_PG_SCHEMA_INVALID", location, error.message)
        )
    return diagnostics


def _producer_export_schema_for_local_validation(
    schema: dict[str, Any],
) -> dict[str, Any]:
    """Keep vendored bytes immutable while validating CE-owned bundle semantics."""
...
def validate_project_gate_lock(repo_root: Path) -> list[Diagnostic]:
    ...
    vendored_path = (
        repo_root / "contracts/project-gate/producer-gate-export.v1.schema.json"
    )
    if not lock_path.is_file():
        return [
            Diagnostic(
                "CE_PG_LOCK_MISSING",
                str(lock_path),
                "Project Gate contract lock is missing.",
            )
        ]
    if not vendored_path.is_file():
        return [
            Diagnostic(
                "CE_PG_VENDORED_CONTRACT_MISSING",
                str(vendored_path),
                "Vendored Producer Gate Export contract is missing.",
            )
        ]
    ...
    canonical = (
        lock.get("canonical") if isinstance(lock.get("canonical"), dict) else {}
    )
    vendored = (
        lock.get("vendored") if isinstance(lock.get("vendored"), dict) else {}
    )
    verification = (
    ...
    if canonical.get("repository") != PROJECT_GATE_REPOSITORY:
        diagnostics.append(
            Diagnostic(
                "CE_PG_LOCK_OWNER_DRIFT",
                "canonical.repository",
                "Canonical owner must remain Project Gate.",
            )
        )
    if canonical.get("path") != "contracts/common/producer-gate-export.v1.schema.json":
        diagnostics.append(
            Diagnostic(
                "CE_PG_LOCK_PATH_DRIFT",
                "canonical.path",
                "Canonical path drifted.",
            )
        )
    if canonical.get("commit_sha") != EXPECTED_PROJECT_GATE_COMMIT:
        diagnostics.append(
            Diagnostic(
                "CE_PG_MOVING_REF_FORBIDDEN",
                "canonical.commit_sha",
                "Canonical commit must be the immutable Prompt 0 merge commit.",
            )
        )
    if canonical.get("file_sha256") != EXPECTED_PRODUCER_EXPORT_SHA256:
        diagnostics.append(
            Diagnostic(
                "CE_PG_LOCK_HASH_DRIFT",
                "canonical.file_sha256",
                "Canonical SHA-256 does not match Prompt 0 pin.",
            )
        )
    if vendored.get("repository") != CE_REPOSITORY:
        diagnostics.append(
            Diagnostic(
                "CE_PG_LOCK_VENDOR_REPO_DRIFT",
                "vendored.repository",
                "Vendored repository must be CE.",
            )
        )
    if vendored.get("path") != "contracts/project-gate/producer-gate-export.v1.schema.json":
        diagnostics.append(
            Diagnostic(
                "CE_PG_LOCK_VENDOR_PATH_DRIFT",
                "vendored.path",
                "Vendored path drifted.",
            )
        )
    if vendored.get("file_sha256") != vendored_hash:
        diagnostics.append(
            Diagnostic(
                "CE_PG_VENDORED_HASH_MISMATCH",
                "vendored.file_sha256",
                "Vendored lock hash does not match local file bytes.",
            )
        )
    if vendored_hash != EXPECTED_PRODUCER_EXPORT_SHA256:
        diagnostics.append(
            Diagnostic(
                "CE_PG_VENDORED_BYTES_MISMATCH",
                str(vendored_path),
                "Vendored file bytes do not match the immutable Project Gate contract hash.",
            )
        )
    if vendored.get("local_copy_authoritative") is not False:
        diagnostics.append(
            Diagnostic(
                "CE_PG_LOCAL_COPY_NOT_AUTHORITATIVE",
                "vendored.local_copy_authoritative",
                "Local copy must not be authoritative.",
            )
        )
    if verification.get("byte_equality_required") is not True:
        diagnostics.append(
            Diagnostic(
                "CE_PG_BYTE_EQUALITY_REQUIRED",
                "verification.byte_equality_required",
                "Exact byte equality must be required.",
            )
        )
    if verification.get("compare_against_moving_default_branch") is not False:
        diagnostics.append(
            Diagnostic(
                "CE_PG_MOVING_DEFAULT_BRANCH_FORBIDDEN",
                "verification.compare_against_moving_default_branch",
                "Moving default branch comparison is forbidden.",
            )
        )
    return diagnostics
...
def validate_pipeline_manifest(repo_root: Path) -> list[Diagnostic]:
    ...
    for index, stage in enumerate(stages):
        if not isinstance(stage, dict):
            diagnostics.append(
                Diagnostic(
                    "CE_PIPELINE_STAGE_OBJECT_REQUIRED",
                    f"project_execution_stages[{index}]",
        ...
        if isinstance(stage_id, str):
            ...
            diagnostics.append(
                Diagnostic(
                    "CE_PIPELINE_STAGE_ID_TYPE_INVALID",
                    f"project_execution_stages[{index}].stage_id",
                    "Stage ID must be a string.",
                )
            )
        ...
        if isinstance(ordinal, int) and not isinstance(ordinal, bool):
            ...
            diagnostics.append(
                Diagnostic(
                    "CE_PIPELINE_ORDINAL_TYPE_INVALID",
                    f"project_execution_stages[{index}].ordinal",
                    "Stage ordinal must be an integer.",
                )
            )
        ...
        if stage.get("mandatory") is True and not outputs:
            diagnostics.append(
                Diagnostic(
                    "CE_PIPELINE_MANDATORY_OUTPUT_REQUIRED",
                    f"project_execution_stages[{index}].required_outputs",
                    "Mandatory stages require explicit outputs.",
                )
            )
    if len(stage_ids) != len(set(stage_ids)):
        diagnostics.append(
            Diagnostic(
                "CE_PIPELINE_DUPLICATE_STAGE_ID",
    ...
    if len(ordinals) != len(set(ordinals)):
        diagnostics.append(
            Diagnostic(
                "CE_PIPELINE_DUPLICATE_ORDINAL",
                "project_execution_stages",
                "Stage ordinals must be unique.",
            )
        )
    if ordinals != sorted(ordinals):
        diagnostics.append(
            Diagnostic(
                "CE_PIPELINE_NON_DETERMINISTIC_ORDER",
                "project_execution_stages",
                "Stages must be sorted by ordinal.",
            )
        )
    ...
    if not isinstance(last_stage, dict) or last_stage.get("stage_id") != "project_gate_export":
        diagnostics.append(
            Diagnostic(
                "CE_PIPELINE_EXPORT_NOT_FINAL",
                "project_execution_stages[-1].stage_id",
                "Project Gate export must be the final stage.",
            )
        )
    return diagnostics


def validate_ce_stage_payload(
    repo_root: Path,
    ...
    if status in {"blocked", "needs_user_evidence", "needs_architect_amendment"} and emitted:
        diagnostics.append(
            Diagnostic(
                "CE_PAYLOAD_BLOCKED_BUILDER_PACKAGE",
                "builder_package_emitted",
                "Blocked CE outcomes must not emit Builder packages.",
            )
        )
    identity = (
        payload.get("architecture_identity")
        if isinstance(payload.get("architecture_identity"), dict)
        else {}
    )
    if identity.get("selected_candidate_id_unchanged") is not True:
        diagnostics.append(
            Diagnostic(
                "CE_PAYLOAD_SELECTED_CANDIDATE_CHANGED",
                "architecture_identity.selected_candidate_id_unchanged",
                "selected_candidate_id must remain unchanged.",
            )
        )
    if identity.get("build_tree_identity_preserved") is not True:
        diagnostics.append(
            Diagnostic(
                "CE_PAYLOAD_BUILD_TREE_NOT_PRESERVED",
                "architecture_identity.build_tree_identity_preserved",
                "Build Tree identity must remain preserved.",
            )
        )
    return diagnostics
...
def validate_stage_bundle(bundle: dict[str, Any]) -> list[Diagnostic]:
    ...
    if bundle.get("schema_version") != "stage-evidence-bundle.v1":
        diagnostics.append(
            Diagnostic(
                "CE_STAGE_BUNDLE_SCHEMA_VERSION_INVALID",
                "final_stage_bundle.schema_version",
                "Stage Bundle v1 must be reused.",
            )
        )
    if bundle.get("stage") != "ce":
        diagnostics.append(
            Diagnostic(
                "CE_STAGE_BUNDLE_WRONG_STAGE",
                "final_stage_bundle.stage",
                "Stage Bundle stage must be ce.",
            )
        )
    payload_schema = (
        bundle.get("payload_schema")
        if isinstance(bundle.get("payload_schema"), dict)
        else {}
    )
    if payload_schema.get("id") != "ev4-ce-stage-payload@1.0.0":
        diagnostics.append(
            Diagnostic(
                "CE_STAGE_BUNDLE_PAYLOAD_SCHEMA_INVALID",
                "final_stage_bundle.payload_schema.id",
                "Final Stage Bundle must carry CE Stage Payload v1.",
            )
        )
    produced_by = (
        bundle.get("produced_by")
        if isinstance(bundle.get("produced_by"), dict)
        else {}
    )
    if produced_by.get("repository") != CE_REPOSITORY:
        diagnostics.append(
            Diagnostic(
                "CE_STAGE_BUNDLE_PRODUCER_INVALID",
                "final_stage_bundle.produced_by.repository",
                "Stage Bundle producer must be CE.",
            )
        )
    ...
    if not isinstance(declared_synthetic, bool) or declared_synthetic != derived_synthetic:
        diagnostics.append(
            Diagnostic(
                "CE_STAGE_BUNDLE_SYNTHETIC_STATE_MISMATCH",
                "final_stage_bundle.synthetic",
                "Declared synthetic state must equal validator-derived carried evidence state.",
            )
        )
    if not derived_synthetic and produced_by.get("commit_sha") is None:
        diagnostics.append(
            Diagnostic(
                "CE_STAGE_BUNDLE_COMMIT_REQUIRED",
                "final_stage_bundle.produced_by.commit_sha",
                "Non-synthetic bundles require producer commit evidence.",
            )
        )
    payload = (
    ...
    if payload.get("schema_id") != "ev4-ce-stage-payload@1.0.0":
        diagnostics.append(
            Diagnostic(
                "CE_STAGE_BUNDLE_PAYLOAD_ID_INVALID",
                "final_stage_bundle.payload.schema_id",
                "Stage payload schema_id must be CE Stage Payload v1.",
            )
        )
    return diagnostics


def validate_producer_gate_export(
    repo_root: Path,
    export: dict[str, Any],
) -> list[Diagnostic]:
    ...
    producer = (
        export.get("producer") if isinstance(export.get("producer"), dict) else {}
    )
    if producer.get("stage") != "ce" or producer.get("repository") != CE_REPOSITORY:
        diagnostics.append(
            Diagnostic(
                "CE_PG_EXPORT_PRODUCER_INVALID",
                "producer",
                "Producer identity must be CE.",
            )
        )
    acquisition = (
        export.get("acquisition_mode")
        if isinstance(export.get("acquisition_mode"), dict)
        else {}
    )
    if acquisition.get("silent_fallback_allowed") is not False:
        diagnostics.append(
            Diagnostic(
                "CE_PG_SILENT_FALLBACK_FORBIDDEN",
    ...
    if acquisition.get("mode") != "producer_emitted_gate_artifact":
        diagnostics.append(
            Diagnostic(
                "CE_PG_ACQUISITION_MODE_INVALID",
                "acquisition_mode.mode",
                "Producer-emitted gate artifact mode is mandatory.",
            )
        )
    ...
    if not isinstance(last_export_stage, dict) or last_export_stage.get("stage_id") != "project_gate_export":
        diagnostics.append(
            Diagnostic(
                "CE_PG_EXPORT_STAGE_NOT_FINAL",
                "stage_manifest[-1].stage_id",
                "Project Gate export stage must be final.",
            )
        )
    for index, stage in enumerate(stage_manifest):
        if not isinstance(stage, dict):
            diagnostics.append(
                Diagnostic(
                    "CE_PG_STAGE_OBJECT_REQUIRED",
                    f"stage_manifest[{index}]",
                    "Each export stage entry must be an object.",
                )
            )
            continue
        if stage.get("status") == "complete":
            ...
            if output.get("present") is not True or not output.get("artifact_ref"):
                diagnostics.append(
                    Diagnostic(
                        "CE_PG_COMPLETE_STAGE_OUTPUT_REQUIRED",
                        f"stage_manifest[{index}].output",
                        "Complete stages require a real output reference.",
                    )
                )

    handoff = (
        export.get("handoff") if isinstance(export.get("handoff"), dict) else {}
    )
    if handoff.get("allowed") is True:
        if handoff.get("status") not in {"successful", "successful_with_flags"}:
            diagnostics.append(
                Diagnostic(
                    "CE_PG_HANDOFF_ALLOWED_STATUS_INVALID",
                    "handoff.status",
                    "Allowed handoff requires successful status.",
                )
            )
        if handoff.get("blocking_diagnostics"):
            diagnostics.append(
                Diagnostic(
                    "CE_PG_HANDOFF_ALLOWED_WITH_BLOCKERS",
                    "handoff.blocking_diagnostics",
                    "Allowed handoff requires zero blocking diagnostics.",
                )
            )
    elif not handoff.get("failure_reasons"):
        diagnostics.append(
            Diagnostic(
                "CE_PG_HANDOFF_FAILURE_REASON_REQUIRED",
                "handoff.failure_reasons",
                "Disallowed handoff requires structured failure reasons.",
            )
        )
    ...
    derived_synthetic = derive_stage_bundle_synthetic(bundle)
    if handoff.get("allowed") is True and derived_synthetic:
        diagnostics.append(
            Diagnostic(
                "CE_PG_SYNTHETIC_EVIDENCE_BLOCKS_HANDOFF",
                "final_stage_bundle.synthetic",
                "Validator-derived synthetic evidence cannot authorize Builder handoff.",
            )
        )
    payload_data = (
        (bundle.get("payload") or {}).get("data")
    ...
    if isinstance(payload_data, dict):
        ...
        diagnostics.append(
            Diagnostic(
                "CE_PG_PAYLOAD_DATA_MISSING",
                "final_stage_bundle.payload.data",
                "Final Stage Bundle must include CE payload data.",
            )
        )
...
def validate_repository(repo_root: Path) -> dict[str, Any]:
    ...
    for fixture in sorted((repo_root / "fixtures/project_gate_export").glob("*.json")):
        ...
        if expected_pass is not None and passed is not bool(expected_pass):
            diagnostics.append(
                Diagnostic(
                    "CE_PG_FIXTURE_EXPECTATION_MISMATCH",
                    str(fixture),
                    f"Fixture expected validation_pass={expected_pass}, got {passed}.",
                )
            )
        expected_rules = set(expected.get("rules_violated") or [])
        actual_rules = {item.code for item in fixture_diagnostics}
        for code in sorted(expected_rules - actual_rules):
            diagnostics.append(
                Diagnostic(
                    "CE_PG_EXPECTED_RULE_MISSING",
                    str(fixture),
                    f"Expected diagnostic was not emitted: {code}.",
                )
            )
    return {
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate CE Project Gate producer adoption artifacts."
    )
