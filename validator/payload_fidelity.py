...
from .runtime_obligations import (
...
def _phase_aware_dependency_result(
    ...
    for row in rows:
        ...
        if policy["lifecycle_phase"] == POST_BUILDER_RUNTIME:
            # A complete obligation satisfies the Builder boundary but never claims execution.
            row["blocking"] = False
            row["evidence_refs"] = []
    obligations = derive_runtime_obligations(rows)
    pre_builder_rows = [
        row
        for row in rows
        if row["lifecycle_phase"] != POST_BUILDER_RUNTIME
    ]
    result["rows"] = rows
    result["blocking_dependencies"] = sorted(
        f"{row['subject_ref']}:{row['claim_id']}:{row['status']}"
        for row in pre_builder_rows
        if row.get("status") not in {"satisfied", "not_applicable"}
    )
    result["unresolved_evidence"] = [
        {
            "unresolved_id": f"unresolved-{row['subject_ref']}-{row['claim_id']}",
            "claim_ref": f"{row['subject_ref']}:{row['claim_id']}",
            "owner": row["authority_owner"],
            "reason": row["status"],
    ...
    result["status"] = (
        "blocked"
        if result["blocking_dependencies"] or result.get("diagnostics")
        else "complete"
    )
    return json.loads(canonical_bytes(result)), obligations


def _action_ir_with_effects(
    action_ir: Sequence[Mapping[str, Any]], obligations: Mapping[str, Any]
) -> list[dict[str, Any]]:
    effects = obligations.get("action_effects")
    records = effects.get("records") if isinstance(effects, Mapping) else []
    by_id = {
        str(item.get("action_id")): dict(item)
    ...
    for item in action_ir:
        ...
        value["derived_effects"] = {
            **copy.deepcopy(dict(value.get("derived_effects") or {})),
            "class_result": copy.deepcopy(effect.get("class_effect")),
            "structure_result": copy.deepcopy(effect.get("structure_effect")),
            "forbidden_work_conflicts": copy.deepcopy(
                list(effect.get("forbidden_work_conflicts") or [])
            ),
            "blocked": bool(effect.get("blocked")),
        }
        result.append(value)
...
def _extension(payload: dict[str, Any], kind: str, value: Any) -> None:
    payload.setdefault("extension_records", []).append(
        {"kind": kind, "result": copy.deepcopy(value)}
    )


def evaluate_ce_transaction(
    *,
    ...
    """Canonical API/CLI/exporter evaluation path for Builder readiness.

    Runtime declarations create obligations. They never create execution evidence. The resulting
    Payload may be CE-complete and Builder-ready while runtime validation and final Project Gate
    ...
    dependency, runtime_obligations = _phase_aware_dependency_result(
    ...
    ir_hidden = sorted(
        {
            str(path)
            for item in action_ir
            for path in item.get("hidden_decision_paths") or []
        }
    )
    if ir_hidden:
        ...
        strategy.setdefault("diagnostics", []).append(
            {"code": "CE_STRATEGY_HIDDEN_BUILDER_DECISION", "paths": ir_hidden}
        )
    ...
    results["claim_classification"] = {
        claim_id: {
            "lifecycle_phase": policy["lifecycle_phase"],
            "builder_handoff_effect": policy["builder_handoff_effect"],
            "final_completion_effect": policy["final_completion_effect"],
        }
        for claim_id, policy in sorted(CLAIM_POLICIES.items())
    }
    ...
    payload = assemble_ce_stage_payload(
        ...
        results["strategy_result"],
        {
            ...
            "review_draft": normalized_draft,
            "runtime_execution_requests": [
                dict(item) for item in runtime_execution_requests
            ],
    ...
    lifecycle = lifecycle_status(
        builder_ready=builder_ready, obligations=runtime_obligations
    )
    ...
    if isinstance(package, dict):
        ...
        # The public Builder action shape is retained, but every parameter comes from Action IR.
        package["first_safe_builder_batch"]["actions"] = [
            {
                "action_id": item["action_id"],
                "action_type": item["action_type"],
                "target_node": item["target_node"],
                "parameters": copy.deepcopy(item["normalized_parameters"]),
                "requires_decision": item["decision_state"] != "resolved",
            }
            for item in action_ir
        ]
    payload["boundary_assertions"]["ce_did_not_claim_responsive_completion"] = True
    ...
    payload["authority_resolution_digest"] = sha256_json(
...
def recompute_expected_payload(
    *,
    ...
    try:
        return evaluate_ce_transaction(
            architect_intake=architect_intake,
            source_bundle=source_bundle,
            ...
            diagnostics.append(
                {
                    ...
                    "path": f"$.{key}",
                    "message": (
                        f"Persisted {key} differs from deterministic recomputation."
                    ),
                }
            )
    if not diagnostics:
...
def validate_export_fidelity(
    export: Mapping[str, Any], **kwargs: Any
) -> dict[str, Any]:
    payload = (
...
__all__ = [
    "PayloadFidelityError",
    "assert_payload_fidelity",
    "compare_persisted_payload",
    "evaluate_ce_transaction",
    "recompute_expected_payload",
    "validate_export_fidelity",
    "validate_payload_fidelity",
]
