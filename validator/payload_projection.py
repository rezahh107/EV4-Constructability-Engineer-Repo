...
def _required_value(
    row: Mapping[str, Any] | None, *, responsive: bool = False
) -> bool | str | None:
    if row is None:
    ...
    if responsive:
        ...
        if row.get("status") == "downstream_validation_required":
            return "unknown"
        return "blocked"
...
def _node_status(
    ...
    blockers = [
        str(value)
        for value in dependency_result.get("blocking_dependencies") or []
        if str(value).startswith(f"{node_id}:")
    ]
    amendment_nodes = {
        str(value) for value in strategy_result.get("architect_amendment_nodes") or []
    }
    if node_id in amendment_nodes:
        return (
            "needs_architect_amendment",
            "Architect permission is required for class/structure change.",
        )
    if blockers:
...
def _derived_actions(
    review_draft: Mapping[str, Any], strategy_result: Mapping[str, Any]
) -> list[dict[str, Any]]:
    del review_draft
    action_ir = strategy_result.get("action_ir")
    if not isinstance(action_ir, list):
        return []
    result: list[dict[str, Any]] = []
    for item in action_ir:
        if not isinstance(item, Mapping):
            continue
        parameters = item.get("normalized_parameters")
        ...
        result.append(
            {
                ...
                "requires_decision": item.get("decision_state") != "resolved",
            }
        )
...
def _strategy_map(
    ...
    *,
    ...
    result["strategy_map_id"] = str(
        result.get("strategy_map_id") or f"strategy-{payload_id[-16:]}"
    )
    ...
    hidden_paths = [
        str(value) for value in strategy_result.get("hidden_builder_decisions") or []
    ]
    ...
    if isinstance(strategies, list):
        for index, strategy in enumerate(strategies):
            ...
            strategy["builder_decisions_required"] = sum(
                path.startswith(prefix) for path in hidden_paths
            )
            strategy["architect_amendment_required"] = str(
