...
|---|---|---:|---|---|---|---|---|---|---|---|
| `R-CE-PAR-01` | Visual references must be lowered into a locked paradigm contract before Builder-ready output. | Critical | `protocols/REFERENCE_PARADIGM_LOCK.md` | `reference_paradigm_lock`, `paradigm_to_structure_map` | `validator.reference_paradigm_lock`, `R29-R32` | `tests/valid/center_anchored_symmetric_pill_cards.json` | `tests/invalid/missing_structure_map.json` | `npm run test:reference-paradigm-lock` | Builder intake pending | `ci_enforced` |
...
| `R-CE-ISM-01` | Implementation strategy map should not leave hidden Builder decisions. | High | `docs/IMPLEMENTATION_STRATEGY_GATE.md` | `implementation_strategy_map.strategies[*].builder_decisions_required` | `None` | existing schema examples | `None` | `None` | Builder must not choose strategy | `schema_backed` |
...
| `R-CE-BATCH-01` | First Builder batch must not hide unresolved decisions in action parameters. | High | `docs/ROLE_BOUNDARIES.md`, Builder Package Gate | `first_safe_builder_batch.actions[*].parameters` | `None` | existing valid package fixtures | `None` | `None` | Builder should execute, not decide | `schema_backed` |
