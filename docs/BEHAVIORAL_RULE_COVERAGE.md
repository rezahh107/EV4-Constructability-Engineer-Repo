...
| `R-CE-ISM-01` | Implementation strategy map must leave zero Builder decisions. | Critical | `docs/IMPLEMENTATION_STRATEGY_GATE.md` | `implementation_strategy_map.strategies[*].builder_decisions_required` | `None` | existing schema examples | `None` | `None` | Builder must not choose strategy | `schema_backed` |
