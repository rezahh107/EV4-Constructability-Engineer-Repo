...
def test_official_cli_remains_verified_review_runtime() -> None:
    ...
    source = (ROOT / "validator/verified_project_gate_exporter.py").read_text(
        encoding="utf-8"
    )
    for option in (
        "--review-draft",
...
def test_one_canonical_evaluator_and_no_parallel_carrier_authority() -> None:
    ...
    verified = (ROOT / "validator/verified_constructability.py").read_text(
        encoding="utf-8"
    )
...
def test_legacy_payload_cannot_authorize_builder(tmp_path: Path) -> None:
    result = reject_legacy_payload_export(
        repo_root=ROOT,
        payload_path=tmp_path / "legacy.json",
        source_intake_path=tmp_path / "intake.json",
        source_bundle_path=tmp_path / "bundle.json",
...
def test_runtime_result_from_another_transaction_is_not_selected(tmp_path: Path) -> None:
    ...
    transaction = execution_transaction_id(
        ...
        review_draft=_responsive_draft(),
        requests=[request],
    )
    ...
    row = evaluate_claim(
        ...
        execution_transaction_id_override="other-transaction",
    )
...
def test_successful_runtime_execution_removes_pending_obligation(tmp_path: Path) -> None:
    target = _runtime_target(tmp_path / "responsive.json")
    payload, results = evaluate_ce_transaction(
        architect_intake=canonical_intake(),
        source_bundle=canonical_bundle(),
        review_draft=_responsive_draft(),
        repo_root=tmp_path,
        runtime_execution_requests=[_runtime_request(target.name)],
    )
    runtime_row = next(
        row
        for row in results["dependency_result"]["rows"]
        if row["claim_id"] == "responsive_behavior"
    )
...
def test_missing_runtime_target_remains_phase_aware_obligation(tmp_path: Path) -> None:
    payload, _ = evaluate_ce_transaction(
        architect_intake=canonical_intake(),
        source_bundle=canonical_bundle(),
        review_draft=_responsive_draft(),
        repo_root=tmp_path,
        runtime_execution_requests=[_runtime_request("missing.json")],
    )
...
def test_explicit_snapshot_and_alias_boundaries(tmp_path: Path) -> None:
    ...
    snapshot = capture_json_snapshot(
        review,
        label="CE Review Draft",
        read_error_code="READ",
        changed_error_code="CHANGED",
    )
...
def test_dirty_git_state_is_metadata_only() -> None:
    provenance = GitProvenance(
        ...
        ref="agent/verified-constructability-proof-runtime",
        commit_sha="1" * 40,
        dirty=True,
        dirty_paths=("docs/unrelated.md",),
    )
    verified_payload = {
        ...
        "constructability_review": {"constructability_status": "executable_ready"},
    }
    diagnostics = _handoff_diagnostics(
        verified_payload,
